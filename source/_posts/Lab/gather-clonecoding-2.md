...
이제 어느 위치에 어떤 타일을 배치할지 json으로 일러주면 되는데, 처음에는 이걸 손으로 하나하나 해야하는 줄 알고 포기할까 했는데 ㅎㅎ 어렵지 않게 툴을 찾을 수 있었다. [고마워요 Tiled](https://www.mapeditor.org/) 위에서 모아둔 애셋 타일셋을 넣고 한칸 한칸 그려주면 된다.
